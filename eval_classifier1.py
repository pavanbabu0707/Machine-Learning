...
def logistic_regression_iris():
    # Load the iris dataset and select features (petal length/width)
    iris = load_iris()
    ...
    model.save_model("model1_parameters.json")

    # Test model performance on the test set
    accuracy = model.score(X_test_scaled, y_test)
    print(f"Test set accuracy (petal length/width): {accuracy:.2%}")
